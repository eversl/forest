...
(Rule CompilationUnit (+ (? (> PackageDeclaration)) (* (> ImportDeclaration)) (* (> TypeDeclaration)) ;($ (! EndOfFile) "top level definition expected")
                         ))
...
(Rule ClassBody (@ "classbody" (braces$ (* ClassBodyDeclaration))))
...
(Rule InterfaceDeclaration (@.$: Modifiers "interface" Identifier (? (@$: "extends" (list Type))) (braces$ (* InterfaceBodyDeclaration))))
...
(Rule FormalParameters (@ "formal-param" (parens$ (? FormalParameterDecls))))

(Rule FormalParameter (+ VariableModifiers Type VariableDeclaratorId))

(Rule FormalParameterDecls (+ VariableModifiers Type FormalParameterDeclsRest))

(Rule FormalParameterDeclsRest (/ (+ VariableDeclaratorId (? (+ comma FormalParameterDecls)))
                                  (+ (tok "...") VariableDeclaratorId)))
